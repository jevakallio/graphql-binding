...
const argv = yargs
  ...
  .options({
    ...
    language: {
      alias: 'l',
      describe:
        'Type of the generator. Available languages: typescript, flow, javascript',
      type: 'string',
    },
    outputBinding: {
      alias: 'b',
...
async function run(argv) {
  ...
  const generatorInstance = getGeneratorForLanguage(language, args)
...
function getGeneratorForLanguage(language, args) {
  switch (language) {
    case 'typescript':
